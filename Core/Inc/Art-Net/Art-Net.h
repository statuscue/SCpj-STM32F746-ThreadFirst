...
// updated 1/8/17 for additional ArtCommand commands
//
//////////////////////////////////////////////////////////////
...
#define RcFactoryRes    0x0010	// Factory reset occured
#define RcConfigError 	0x0011	//Product has not been factory configured or commissioned.
#define RcPortAddressOk	0x0012	//Confirms that Port-Address programming via ArtAddress accepted


// Defines for ArtAddress Command field

#define AcNone          0x00    // No Action
#define AcCancelMerge   0x01    // The next ArtDmx packet cancels Node's merge mode
#define AcLedNormal     0x02    // Node front panel indicators operate normally
#define AcLedMute       0x03    // Node front panel indicators are muted
#define AcLedLocate     0x04    // Fast flash all indicators for locate
#define AcResetRxFlags  0x05    // Reset the receive DMX flags for errors, SI's, Text & Test packets
#define AcAnalysisOn    0x06    // Product signal analysis enabled
#define AcAnalysisOff   0x07    // Product signal analysis disabled

#define AcFailHold		0x08    // Failsafe mode = hold last state
#define AcFailZero		0x09    // Failsafe mode = clear outputs
#define AcFailFull		0x0a    // Failsafe mode = outputs to full
#define AcFailScene		0x0b    // Failsafe mode = playback failsafe scene
#define AcFailRecord	0x0c    // Failsafe mode = record current output as failsafe scene
...
#define AcStyleDelta0		0xa0		// Set output style to delta on output port 0
#define AcStyleDelta1		0xa1		// Set output style to delta on output port 1
#define AcStyleDelta2		0xa2		// Set output style to delta on output port 2
#define AcStyleDelta3		0xa3		// Set output style to delta on output port 3

#define AcStyleConstant0	0xb0		// Set output style to continuous on output port 0
#define AcStyleConstant1	0xb1		// Set output style to continuous on output port 1
#define AcStyleConstant2	0xb2		// Set output style to continuous on output port 2
#define AcStyleConstant3	0xb3		// Set output style to continuous on output port 3

#define AcRdmEnable0		0xc0		// Enable RDM on output port 0
#define AcRdmEnable1		0xc1		// Enable RDM on output port 1
#define AcRdmEnable2		0xc2		// Enable RDM on output port 2
#define AcRdmEnable3		0xc3		// Enable RDM on output port 3

#define AcRdmDisable0		0xd0		// Disable RDM on output port 0
#define AcRdmDisable1		0xd1		// Disable RDM on output port 1
#define AcRdmDisable2		0xd2		// Disable RDM on output port 2
#define AcRdmDisable3		0xd3		// Disable RDM on output port 3
...
typedef struct S_ArtPollReply
	...
	uchar GoodOutputA[MaxNumPorts];
	// bit 7 data is transmitting
	// bit 6 data includes test packets
	// bit 5 data includes SIP's
	// bit 4 data includes text
	...
	uchar Status2;            // bit 0 = 0 Node does not support web browser
                              ...
                              // bit 5 = 0 Node is not squawking
							  // bit 5 = 1 Node is squawking

                              // bit 6 = 0 Node is not able to switch output style by ArtCommand
                              // bit 6 = 1 Node is able to switch output style by ArtCommand

                              // bit 7 = 0 Node is not able to enable or disable RDM by ArtCommand
                              // bit 7 = 1 Node is able to enable or disable RDM by ArtCommand

	uchar GoodOutputB[MaxNumPorts];
                    // bit 7 = 1 RDM is disabled.
                    // bit 7 = 0 RDM is enabled.

                    // bit 6 = 1 Output style is continuous.
                    // bit 6 = 0 Output style is delta.

					// bit 5 - 0 = 0

	uchar Status3;            // bit 76 = 00 If network data is lost, it will hold last state
							  // bit 76 = 01 If network data is lost, it will set all outputs to off state
							  // bit 76 = 10 If network data is lost, it will set all outputs to full on
							  // bit 76 = 11 If network data is lost, it will playback the fail-over scene

							  // bit 5 = 0 Node is not able to control failsafe mode by ArtCommand
							  // bit 5 = 1 Node is able to control failsafe mode by ArtCommand

							  // bit 4-0 = Transmit zero, receivers do not tests


	uchar Filler[21]; // Filler bytes, currently zero.
